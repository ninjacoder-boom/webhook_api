 ...
 */
async function updateInternalBhavDE({ phone, tenure, roiDisplay, maturityAmount, finalFDAmount }) {
  try {
    const token = await getSFMCToken();
    ...
    const payload = [
      {
        ...
        values: {
          ...
          Final_FD_Amount:  finalFDAmount != null ? String(finalFDAmount) : ""  // partial or full amount
        }
      }
    ];
    ...
    log("INFO", `Internal_bhav updated | Phone: ${phone} | Tenure: ${tenure} | ROI: ${roiDisplay}% | Maturity_Amount: ${maturityAmount} | Final_FD_Amount: ${finalFDAmount}`);
  } catch (err) {
    if (err.response?.status === 401) tokenCache = { value: null, expiresAt: null };
...
/**
 * Update Internal_bhav_FDconfirmation DE.
 *
 * Status = 'True'  -> partial <= full (or amountType=full):
 *   writes FD_Amount, Tenure, ROI, Maturity_Amount, Status
 *
 * Status = 'False' -> partial > full (invalid):
 *   writes only FD_Amount (partial entered), Tenure, Status
 *   ROI and Maturity_Amount are left blank / untouched
 *
 * Subscriber_Key, Mobile, Name are primary keys / pre-existing -- never overwritten.
 */
async function upsertFDConfirmationDE({
  phone, fdAmount, tenure, roiDisplay, maturityAmount, status
}) {
  try {
    const token = await getSFMCToken();

    // Build values conditionally based on status
    const values =
      status === "True"
        ? {
        ...
        : {
            // partial > full -- only record what was submitted + flag as failed
            FD_Amount: fdAmount != null ? String(fdAmount) : "",
            Tenure:    tenure   || "",
            Status:    "False"
          };
    ...
    log("INFO", `Internal_bhav_FDconfirmation updated | Phone: ${phone} | Status: ${status} | FD_Amount: ${fdAmount} | Tenure: ${tenure} | ROI: ${roiDisplay ?? "n/a"} | Maturity: ${maturityAmount ?? "n/a"}`);
  } catch (err) {
    if (err.response?.status === 401) tokenCache = { value: null, expiresAt: null };
      ...
      for (const change of entry.changes || []) {
        ...
        for (const message of messages) {
          ...
          if (message.type === "interactive" && message.interactive?.type === "nfm_reply") {
            ...
            // ── Step 1: Always fetch full FD amount from Internal_bhav ─────────
            log("INFO", `Fetching FD record from Internal_bhav for: ${from}`);
            const fdRecord  = await fetchFDRecordFromDE(from);
            const fullAmount = fdRecord?.fd_amount ? parseFloat(fdRecord.fd_amount) : null;

            // ── Step 2: Determine finalAmount + validate partial ──────────────
            let finalAmount    = null;
            let partialInvalid = false;   // true when partial > full

            if (amountType === "full") {
              finalAmount = fullAmount;
              log("INFO", `amount_type=full | FD Amount: ${finalAmount}`);

            } else {
              // partial — validate against fullAmount
              const partial = partialAmount ? parseFloat(partialAmount) : null;

              if (partial === null || fullAmount === null) {
                // Cannot validate — missing data, skip everything
                log("WARN", `Partial validation skipped — missing values | partial: ${partial} | full: ${fullAmount}`);
                continue;
              } else if (partial > fullAmount) {
                // Invalid — save FD_Amount + Tenure + Status:False, skip all other DEs
                log("WARN", `Partial (${partial}) exceeds full FD amount (${fullAmount}) — saving Status:False to FDconfirmation for: ${from}`);
                upsertFDConfirmationDE({
                  phone:         from,
                  fdAmount:      partial,   // record what the user entered
                  tenure,
                  roiDisplay:    null,
                  maturityAmount: null,
                  status:        "False"
                ...
                finalAmount = partial;
                log("INFO", `amount_type=partial | Partial: ${partial} <= Full: ${fullAmount} — proceeding`);
              }
            }

            // ── Step 3: Calculate ROI + Maturity Amount ───────────────────────
            let roi            = null;
            let roiDisplay     = null;   // "6", "6.5", "7", "7.5"
            let maturityAmount = null;

            if (tenure && finalAmount) {
              const calc = calculateMaturity(finalAmount, tenure);
              if (calc) {
                roi            = calc.roi;
            ...
            // ── Step 5: Save to FD_Flow DE (non-blocking) ─────────────────────
            ...
            // ── Step 6: Upsert Tenure + ROI + Maturity + Final_FD_Amount back to Internal_bhav ──
            if (tenure && roiDisplay !== null && maturityAmount !== null) {
              updateInternalBhavDE({ phone: from, tenure, roiDisplay, maturityAmount, finalFDAmount: finalAmount });
            }

            // ── Step 7: Update FD_Amount, Tenure, ROI, Maturity_Amount, Status in Internal_bhav_FDconfirmation ─
            // Subscriber_Key, Mobile, Name already exist — only the calculated fields + Status are written
            if (roiDisplay !== null && maturityAmount !== null) {
              upsertFDConfirmationDE({
                ...
                roiDisplay,
                maturityAmount,
                status:        "True"   // partial <= full (or full amount) — valid
              });
            }
          }
        }
